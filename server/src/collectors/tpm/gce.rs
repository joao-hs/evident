...
use crate::{
    collectors::{error::EvidenceCollectionError, nonce, tpm::SoftwareEvidenceCollector},
    domain::evidence::SoftwareEvidence,
};

// const GCE_NV_INDEX_RSA_2048_EK_CERT: u32 = 0x1C00002;
// const GCE_NV_INDEX_ECC_P256_EK_CERT: u32 = 0x1C0000A;
// const GCE_NV_INDEX_RSA_2048_AK_CERT: u32 = 0x1C10000;
// const GCE_NV_INDEX_RSA_2048_AK_TEMPLATE: u32 = 0x1C10001;
const GCE_NV_INDEX_ECC_P256_AK_CERT: u32 = 0x1C10002;
const GCE_NV_INDEX_ECC_P256_AK_TEMPLATE: u32 = 0x1C10003;
const GCE_NV_INDEX_ECC_P256_AK_PERSISTENT_HANDLE: u32 = 0x81018000;
...
impl GceTpmWrapper {
    pub fn new() -> Result<Self, EvidenceCollectionError> {
    ...
    fn create_primary_ecc_ak(
        context: &mut Context,
    ) -> Result<(KeyHandle, Public), EvidenceCollectionError> {
        debug!("Starting creation of primary ECC AK...");

        let template_handle = NvIndexTpmHandle::new(GCE_NV_INDEX_ECC_P256_AK_TEMPLATE)?;
        debug!(
        ...
        let nv_read_result = context.nv_read(
            auth_handle,
            template_handle,
            u16::try_from(template_data_size).map_err(|_| {
                EvidenceCollectionError::InternalError(
                    "(tpm:ak:ecc) template size could not be converted to u16".to_string(),
                )
            })?,
            0, // offset
        )?;
        ...
        context.evict_control(
            ...
            PersistentTpmHandle::new(GCE_NV_INDEX_ECC_P256_AK_PERSISTENT_HANDLE)
                .map_err(|_| {
                    EvidenceCollectionError::InternalError(
                        "(tpm:ak) unable to create persistent tpm handle".to_string(),
                    )
                })?
                .into(),
        )?;
        debug!(
            ...
            GCE_NV_INDEX_ECC_P256_AK_PERSISTENT_HANDLE
        );

        let ak_handle: KeyHandle = context
            .execute_without_session(|ctx| {
                ctx.tr_from_tpm_public(TpmHandle::try_from(
                    GCE_NV_INDEX_ECC_P256_AK_PERSISTENT_HANDLE,
                )?)
            })?
            .into();
        ...
        match ak_loaded_public {
            Public::Ecc {
                ...
                let _ecc_public_key =
                    PublicKey::from_sec1_bytes(&public_key_bytes).map_err(|e| {
                        EvidenceCollectionError::InternalError(format!(
                            "(tpm:ak:ecc) failed to parse ECC AK public key bytes: {:?}",
                            e
                        ))
                    })?;
                debug!(
            ...
            _ => {
                return Err(EvidenceCollectionError::InternalError(
                    "(tpm:ak:ecc) loaded AK is not of ECC type".to_string(),
                ));
            }
        }
    ...
    fn get_ecc_ak_certificate(context: &mut Context) -> Result<Vec<u8>, EvidenceCollectionError> {
        debug!("Starting retrieval of ECC AK certificate...");

        let nv_index_handle = NvIndexTpmHandle::new(GCE_NV_INDEX_ECC_P256_AK_CERT)?;
        debug!(
        ...
        let nv_read_result = context.nv_read(
            auth_handle,
            nv_index_handle,
            u16::try_from(nv_data_size).map_err(|_| {
                EvidenceCollectionError::InternalError(
                    "(tpm:ak:ecc) certificate size could not be converted to u16".to_string(),
                )
            })?,
            0, // offset
        )?;
        ...
        let (_, x509_cert) = X509Certificate::from_der(&ak_cert_data).map_err(|e| {
            EvidenceCollectionError::InternalError(format!(
                "(tpm:ak:ecc) failed to parse ECC AK certificate DER data: {:?}",
                e
            ))
        })?;
    ...
    fn with_new_session<F, R>(context: &mut Context, f: F) -> Result<R, EvidenceCollectionError>
    where
        F: FnOnce(&mut Context) -> Result<R, EvidenceCollectionError>,
    {
...
impl SoftwareEvidenceCollector for GceTpmWrapper {
    fn collect_software_evidence(
        &self,
        nonce: Option<&[u8]>,
    ) -> Result<SoftwareEvidence, EvidenceCollectionError> {
        let nonce = nonce::format_nonce::<32, Sha256>(nonce);

        debug!("Starting software evidence collection...");

        if nonce.len() != 32 {
            debug!(
                "Invalid nonce length: expected 64 bytes, got {} bytes",
                nonce.len()
            );
        ...
        let mut context_guard = self.context.lock().map_err(|_| {
            debug!("Failed to acquire lock on TPM context");
            EvidenceCollectionError::InternalError(
                "(tpm:context:lock) could not obtain lock for using context, possibly another thread panicked while holding the lock".to_string(),
            )
        })?;
