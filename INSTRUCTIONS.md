...
├── config
│   ├── base.yaml
│   ├── lima.yaml      // untracked; generated by `create.sh` from the template `lima.yaml.j2`
│   └── lima.yaml.j2
├── create.sh
├── delete.sh
├── edit.sh
...
The Lima config will be generated at `config/lima.yaml` from the template `config/lima.yaml.j2`.
For this, you need to install the tool `jinja2`. Follow these instructions:
