...
func Task(ctx context.Context, input Input) (Output, error) {
	...
	instanceKeyCert, _, _, _, err := getsnpevidencesubtasks.ValidateAdditionalArtifactsBundle(additionalArtifactsBundle, pb.TargetType_TARGET_TYPE_SNP_GCE)
	if err != nil {
	...
	getEvidenceRequest := &pb.GetEvidenceRequest{}
	{
	...
	// validate TPM Evidence signing key and signing key certificate
	{
		_, _, err := crypto.ParsePublicKey(tpmEvidenceProto.SigningKey)
		if err != nil {
			return zeroOutput, fmt.Errorf("error while parsing signing key in TPM evidence: %w", err)
		}

		akCert, err := crypto.ParseCertificate(tpmEvidenceProto.SigningKey.GetCertificate())
		if err != nil {
			return zeroOutput, fmt.Errorf("error while parsing AK certificate in TPM evidence: %w", err)
		}

		path, err = dot.Store(akCert.Raw)
		if err != nil {
			return zeroOutput, fmt.Errorf("error while storing AK certificate bytes in dot: %w", err)
		}
		log.Get().Debugf("AK certificate bytes stored in dot at path: %s", path)
		output.AkCert = akCert
	}
